...
function Leave() {
  ...
  const handleRejected = async (index, newStatus) => {
    ...
    try {
      await Api.post(
        `api/leaveRequest/reject/${leaveRequest.id}`,
        {},
        { Authorization: `Bearer ${token}` }
      ).then((response) => console.log("put response", response));
      const updatedLeaveRequests = [...leaveRequests];
        ...
        <div className="rounded-t-lg overflow-hidden mt-6 py-6 px-4">
          <table className="min-w-full bg-white rounded-lg">
            <thead className="bg-[#465062] p-4 rounded-lg text-center font-normal text-sm text-white">
              <tr>
                <th className="p-4 text-left font-normal rounded-tl-lg text-sm">
                  Employee ID
                </th>
                ...
                <th className="p-4 text-center font-normal rounded-tr-lg text-sm">
                  Action
                </th>
              </tr>
            </thead>
              ...
              {Array.isArray(leaveRequests) &&
                leaveRequests.map((leaveRequest, index) => (
                  ...
                  >
                    ...
                    <td className="p-4 text-sm">
                      {
                        leaveRequest.employee?.basicDetails?.department
                          ?.departmentName
                      }
                    </td>
                    <td className="p-4 text-sm">
